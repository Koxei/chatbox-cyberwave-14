...
const NewHomeLayout = () => {
  ...
  useEffect(() => {
    // Set current overlay based on route
    if (location.pathname.includes('chatbox')) {
      setCurrentOverlay('chatbox');
    } else if (location.pathname.includes('terminal')) {
      setCurrentOverlay('terminal');
    } else {
  ...
  return (
    <div className="fixed inset-0 bg-deep-sea-blue overflow-hidden">
      ...
      <div className={`fixed inset-0 z-20 pointer-events-none transition-opacity duration-300 ${
