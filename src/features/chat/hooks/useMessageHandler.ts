...
export const useMessageHandler = (
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isImageRequest = userMessage.toLowerCase().startsWith('/image');
      
      if (isImageRequest) {
        const imagePrompt = userMessage.slice(7); // Remove "/image " from the prompt
        ...
        const { data: savedAiMessage, error: aiMessageError } = await supabase
          ...
          .insert([
            {
              content: `Generated image for prompt: ${imagePrompt}`,
              is_ai: true,
              chat_id: currentChat.id,
              user_id: userId,
